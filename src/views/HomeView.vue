...
const donation = computed(() => {
  return donations.value[activeDonationIndex.value] || {
    id: null,
    goal: 0,
    raised: 0,
    text: '',
...
getNews({ page: 1, per_page: 4 }),
getTopVolunteers(),
getAnimals({ page: 1, per_page: 1 }),
getTasks({ status: 'open', page: 1, per_page: 1 }),
])

  if (donationsResult.status === 'fulfilled') {
  donations.value = asList(donationsResult.value)
} else {
  console.error('Не удалось загрузить пожертвования:', donationsResult.reason)
  donations.value = []
}
 ...
 if (topVolunteersResult.status === 'fulfilled') {
  ...
  console.error('Не удалось загрузить топ волонтёров:', topVolunteersResult.reason)
  publicTopVolunteers.value = []
}

  if (animalsResult.status === 'fulfilled') {
    animals.value = asList(animalsResult.value)
  } else {
...
function prevDonation() {
  if (donations.value.length <= 1) return
  activeDonationIndex.value =
    (activeDonationIndex.value - 1 + donations.value.length) % donations.value.length
}
...
const animalsOnCare = computed(() =>
  animals.value.filter(a => a.status !== 'adopted').length
)
  ...
  <div class="container">
    <div class="hero">
      <div>
        ...
        <p class="hero-sub">
          Помогаем приюту быстрее находить волонтёров, собирать пожертвования
          и пристраивать животных в хорошие руки.
        </p>
...
>
  ...
  <div class="don-text">{{ donation.text || 'Сбор помощи приюту' }}</div>
  ...
  <div class="don-footer">
    ...
    <section class="block">
      ...
      <div class="about-grid">
        <article v-for="item in aboutItems" :key="item.title" class="about-card">
      ...
      <article class="news-card">
        <div class="news-image-wrap">
          <img
            ...
            class="news-image"
          />
          <div v-else class="news-image news-image-fallback">
            НОВОСТЬ
          </div>
        </div>
  ...
  <div v-if="news.length > newsPerPage" class="pagination">
    ...
    <button class="page-btn" :disabled="newsPage === newsPages" @click="newsPage++">
      ›
    </button>
  </div>
</section>
          ...
          >
            <div class="top-left">
              ...
              <div>
                ...
                <div class="muted small">Очки: <b>{{ v.points }}</b></div>
              </div>
            </div>
...
.hero-actions{
  animation: heroActionsIn .45s ease both;
}
