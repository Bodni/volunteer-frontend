...
const canSeePoints = computed(() => ['admin', 'volunteer'].includes(effectiveRole.value))
const canSeeRewards = computed(() => ['admin', 'volunteer'].includes(effectiveRole.value))
...
>
  ...
  <span></span>
        ...
        <nav class="nav" :class="{ open: mobileMenuOpen }">
          ...
          <RouterLink to="/help" class="nav-link">Помощь</RouterLink>
          <RouterLink v-if="canSeeRewards" to="/rewards" class="nav-link">Награды</RouterLink>
          <RouterLink v-if="isAuthed" to="/profile" class="nav-link">Профиль</RouterLink>
          <RouterLink v-if="isAuthed && isAdmin" to="/admin" class="nav-link">Админка</RouterLink>
        </nav>
    ...
    <div class="footer-subtitle">
      Платформа волонтёров
        ...
        <div class="footer-text">
          Мы помогаем приюту находить волонтёров, собирать пожертвования
          и быстрее пристраивать животных в хорошие руки.
        </div>

        <div class="footer-copy">
          © 2026 Помощь животным
        </div>
      </div>
